...
void computeHOGs(const Size wsize, const vector<Mat> &img_lst, vector<Mat> &gradient_lst, bool use_flip)
{
    ...
    for(size_t i = 0; i < img_lst.size(); i++)
    {
        ...
        {
            Rect r = Rect((img_lst[i].cols - wsize.width) / 2, (img_lst[i].rows - wsize.height) / 2, wsize.width, wsize.height);
...
void test_trained_detector(string obj_det_filename, string test_dir, string videofilename)
{
    ...
    for (size_t i=0;; i++) {
        ...
        for (size_t j = 0; j < detections.size(); j++)
        {
            ...
            rectangle(img, detections[j], color, 2, 1);
        }
...
int main(int argc, char **argv)
{
    ...
    // string pos_dir = argv[3];
    // string neg_dir = argv[4];
    ...
    // int detector_width = stoi(argv[1]);
    // int detector_height = stoi(argv[2]);
    ...
    else
    {
        for (size_t i = 0; i < pos_lst.size(); ++i)
        {
            ...
            {
                cout << "All positive images should be the same size!" << endl;
                exit(1);
            }
        }
